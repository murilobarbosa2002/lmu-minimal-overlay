...
def test_window_init_windows_os(mock_pygame, mock_time):
    ...
    initial_style = (
        mock_win32con.WS_CAPTION
        | mock_win32con.WS_THICKFRAME
        | mock_win32con.WS_SYSMENU
    )
    mock_win32gui.GetWindowLong.side_effect = lambda hwnd, index: (
        initial_style if index == mock_win32con.GWL_STYLE else 0
    )
    ...
    with patch("sys.platform", "win32"), patch(
        ...
        flags_move = (
            mock_win32con.SWP_NOZORDER
            | mock_win32con.SWP_NOSIZE
            | mock_win32con.SWP_SHOWWINDOW
            | mock_win32con.SWP_NOACTIVATE
        )
        flags_transparency = (
            mock_win32con.SWP_NOMOVE
            | mock_win32con.SWP_NOSIZE
            | mock_win32con.SWP_FRAMECHANGED
        )
        flags_show = (
            mock_win32con.SWP_NOMOVE
            | mock_win32con.SWP_NOSIZE
...
@patch("pygame.display.set_mode")
@patch("pygame.init")
def test_window_init_windows_os_import_error(
    mock_init, mock_set_mode, mock_flip, mock_pump
):
