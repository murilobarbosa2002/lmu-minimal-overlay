# API Reference

Referência completa da API do LMU Telemetry Overlay.

## Telemetry

Sistema de aquisição de dados.

- [Overview](telemetry/index.md)
- [Providers](telemetry/providers.md)
- [Data Models](telemetry/data-models.md)
- [Normalização](telemetry/normalization.md)

## Widgets

Sistema de componentes visuais.

- [Overview](widgets/index.md)
- [Widget Base](widgets/base-widget.md)
- [InputCard](widgets/input-card.md)
- [Criando Widgets](widgets/creating-widgets.md)


## Core

Componentes centrais da arquitetura.

- [State Management](core/state-management.md)

## UI

Componentes de interface e gerenciamento de janela.

- [Window Manager](ui/window-manager.md)

## Configuration

Sistema de configuração.

- [Overview](configuration/index.md)
- [config.json](configuration/config-json.md)
- [layout.json](configuration/layout-json.md)
- [ConfigManager](configuration/config-manager.md)
