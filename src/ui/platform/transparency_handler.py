...
class Win32TransparencyHandler(ITransparencyHandler):
    def __init__(
        self, chroma_key_color: tuple[int, int, int] = (255, 0, 128), alpha: int = 217
    ):
    ...
    def apply_transparency(self, window_handle: int) -> None:
        ...
        try:
            ...
            win32gui.SetWindowLong(
                window_handle, win32con.GWL_EXSTYLE, ex_style | win32con.WS_EX_LAYERED
            )
    ...
    def set_window_pos(
        self, window_handle: int, position_x: int, position_y: int
    ) -> None:
        if sys.platform != "win32":
            return
        try:
